...
public class LabirintoBuilderTest {
	...
	public void testMonolocaleConAttrezzoSingoloDuplicato() {
		...
		assertTrue(size==1);
		assertEquals(Arrays.asList(new Attrezzo("spada",1)),Arrays.asList(monolocale.getStanzaIniziale().getAttrezzi().values().toArray()));
	}
	...
	public void testBilocale() {
		...
		assertEquals(Collections.singletonList(nord),Arrays.asList(bilocale.getStanzaIniziale().getDirezioni().toArray()));
		assertEquals(Collections.singletonList(sud),Arrays.asList(bilocale.getStanzaFinale().getDirezioni().toArray()));
	}
	...
	public void testLabirintoCompletoConTutteLeStanze() {
		...
		assertEquals(Arrays.asList(a1,a2),Arrays.asList(corridoio.getAttrezzi().values().toArray()));
	}
}